...
#### 빈칸 | [651]
DB 연결을 미리 만들어 두고 재사용하는 구조를 ___ 이라 한다.
> 커넥션 풀
> HikariCP는 Spring Boot 2.x부터 기본 커넥션 풀로 채택됐다. 성능과 안정성이 뛰어나 업계 표준으로 사용된다.

#### 빈칸 | [652]
Spring에서 같은 트랜잭션 내 동일 커넥션을 유지하는 핵심 기능은 트랜잭션 ___ 이다.
> 동기화
> TransactionSynchronizationManager는 ThreadLocal에 커넥션을 저장해 같은 스레드에서 동일 커넥션이 재사용되도록 보장한다.
