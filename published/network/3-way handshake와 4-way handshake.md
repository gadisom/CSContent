...
- [3-way handshake]{.term}: TCP에서 데이터를 주고받기 전에 연결을 수립하는 3단계 절차다. [SYN → SYN+ACK → ACK]{.accent} 순서로 진행된다.
- [4-way handshake]{.term}: TCP 연결을 종료하는 4단계 절차다. [FIN → ACK → FIN → ACK]{.accent} 흐름으로 각 방향의 전송을 따로 닫는다.
...
## 핵심 포인트
- [연결 수립의 시작]{.accent}: 클라이언트는 [SYN]{.term}을 보내고 [SYN_SENT]{.term} 상태가 된다. 이때 자신의 초기 시퀀스 번호를 서버에 알린다.
- 서버는 [SYN]{.term}을 받으면 [SYN_RECEIVED]{.term} 상태가 되고, 클라이언트의 SYN에 대한 [ACK]{.term}와 서버 자신의 SYN을 함께 보낸다. 이 패킷이 흔히 [SYN-ACK]{.term}라고 불린다.
- 클라이언트는 서버의 [SYN-ACK]{.term}를 받으면 [ACK]{.term}를 보내고 [ESTABLISHED]{.term} 상태가 된다. 서버도 이 ACK를 받으면 ESTABLISHED 상태가 되어 양쪽 모두 데이터 전송 준비가 끝난다.
- [2단계로 부족한 이유]{.danger}: 양쪽 모두 송신과 수신이 가능한지 확인해야 하기 때문이다. 클라이언트의 마지막 ACK는 서버가 보낸 SYN을 클라이언트가 정상적으로 받았음을 증명한다.
- 연결 수립 단계에서 양쪽은 초기 시퀀스 번호를 교환한다. 이후 TCP는 [시퀀스 번호와 ACK 번호]{.accent}를 이용해 데이터 순서 보장, 손실 감지, 재전송을 수행한다.
- [연결 종료의 시작]{.accent}: 4-way handshake는 보통 능동 종료자가 [FIN]{.term}을 보내면서 시작된다. FIN을 보낸 쪽은 [FIN_WAIT_1]{.term}로 이동하고, 상대의 ACK를 받으면 [FIN_WAIT_2]{.term}에서 상대의 FIN을 기다린다.
- 수동 종료자는 FIN을 받으면 먼저 ACK를 보내고 [CLOSE_WAIT]{.term} 상태가 된다. 이 상태는 [상대는 더 이상 보내지 않지만, 나는 아직 보낼 데이터가 남아 있을 수 있음]{.muted}을 뜻한다.
- 수동 종료자가 남은 데이터를 모두 보낸 뒤 FIN을 보내면 [LAST_ACK]{.term} 상태가 된다. 능동 종료자는 이 FIN에 ACK를 보내고 [TIME_WAIT]{.term} 상태로 들어간다.
- [종료가 4단계인 이유]{.accent}: TCP 연결이 양방향 독립 스트림이기 때문이다. 한쪽이 더 이상 보낼 데이터가 없다고 해서 상대도 즉시 보낼 데이터가 없는 것은 아니다.
- [TIME_WAIT의 역할]{.warning}: 마지막 ACK가 유실되었을 때 상대가 FIN을 재전송할 수 있는 시간을 남긴다. 또한 이전 연결의 지연 패킷이 같은 [4-tuple]{.term} 연결에 섞이는 위험을 줄인다.
- [TIME_WAIT]{.term}는 주로 먼저 종료를 시작한 쪽에 생긴다. 클라이언트가 먼저 닫으면 클라이언트에, 서버가 먼저 닫으면 서버에 생길 수 있다.
- [주의할 상태]{.danger}: 서버에서 [CLOSE_WAIT]{.term}가 오래 남아 있으면 애플리케이션이 소켓을 제대로 닫지 않았다는 신호일 수 있다. 반대로 TIME_WAIT가 많은 것은 TCP 종료 절차의 정상적인 결과일 수 있다.
- TCP 연결은 IP와 포트 번호를 포함한 [4-tuple]{.term}로 식별된다. 같은 클라이언트 IP, 클라이언트 포트, 서버 IP, 서버 포트 조합의 지연 패킷을 구분하는 데 TIME_WAIT가 도움이 된다.
