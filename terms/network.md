...
- detail: DNS에서는 캐시된 응답을 얼마나 오래 재사용할 수 있는지 나타낸다. TTL이 길면 조회는 줄지만 변경 반영이 늦고, 짧으면 반영은 빠르지만 조회가 늘어난다.
- related: [DNS]
- tags: [DNS, 캐시]
...
- detail: 브라우저가 서버에 연결하기 전에 사람이 읽기 쉬운 도메인을 실제 서버 주소로 해석한다.
- related: [DNS, 웹 요청 흐름]
- tags: [도메인, IP 주소]
...
- detail: 브라우저나 운영체제의 요청을 받아 루트, TLD, 권한 있는 네임서버 등을 조회하고 최종 응답을 돌려준다.
- related: [DNS]
- tags: [DNS, 재귀 질의]
...
- detail: 예를 들어 `example.com`이 어떤 IPv4 주소로 가야 하는지 알려준다.
- related: [DNS]
- tags: [DNS, 레코드, IPv4]
...
- detail: 실제 주소를 직접 가리키기보다 다른 정식 이름을 참조하게 만든다.
- related: [DNS]
- tags: [DNS, 레코드, 별칭]
...
- detail: 기본 HTTP는 평문 기반이며, 보안이 필요한 통신은 TLS가 적용된 HTTPS로 보호한다.
- related: [HTTP와 HTTPS, 웹 요청 흐름]
- tags: [웹, 프로토콜]

## HTTPS
- full: HyperText Transfer Protocol Secure
- answer: HTTP 통신을 TLS로 보호하는 방식
- detail: 서버 인증, 암호화, 무결성 검사를 통해 웹 요청과 응답을 안전하게 주고받도록 한다.
- related: [HTTP와 HTTPS, 대칭키와 비대칭키]
- tags: [TLS, 보안]

## TLS
- full: Transport Layer Security
- answer: 통신 구간에 암호화, 무결성, 인증을 제공하는 보안 프로토콜
- detail: HTTPS에서는 TLS 핸드셰이크로 세션 키를 만들고 이후 데이터를 대칭키로 암호화한다.
- related: [HTTP와 HTTPS, 대칭키와 비대칭키]
- tags: [HTTPS, 암호화]
...
- related: [3-way handshake와 4-way handshake]
- tags: [TCP, handshake]

## ACK
- answer: 상대가 보낸 데이터를 확인했다는 의미의 제어 플래그
- detail: TCP에서는 데이터 수신 확인과 연결 수립/종료 단계에서 사용된다.
- related: [3-way handshake와 4-way handshake, TCP와 UDP]
- tags: [TCP, handshake]
